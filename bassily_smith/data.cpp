...
boolSet::boolSet(){
    ...
    // resize col, row
    dataMtx.resize(n, std::vector<bool>(m));
    dataMtx[0][0]=1;
}
 ...
 * OUTPUTS: matrix value at index or -1 on fail
 */
bool boolSet::get_boolMtx(int m_in, int n_in){
    // error checkgin
    if(m_in<0 || m_in>m || n_in<0 || n_in>n)
        return -1;
 ...
 * NOTE: INPUTS ARE 1 INDEXED
 */
int boolSet::set_boolMtx(int m_in, int n_in, bool val){
    // check bounds
    if(m_in<0 || m_in>this->m || n_in<0 || n_in>this->n)
        return -1;
 ...
 * OUTPUTS: 0 success -1 fail
 */
int boolSet::rekeep_boolMtx(int m_in, int n_in){
    // check bounds
    if(m_in==m && n_in==n)
        return 0;
    else if(m_in<0 || n_in<0 || m_in<m || n_in<n)
    ...
    // generate new vector
    if(m_in>m || n_in>n){
        std::vector< std::vector<bool> > asdf(m_in*2, std::vector<bool>(n_in*2, 0.0) );
 ...
 * OUTPUTS: 0 on success -1 on fail
 */
int boolSet::resize_boolMtx(int m_in, int n_in){
    // check bounds
    if(m_in==m && n_in==n){
        // extra clear here
        for(int i=0; i<m_in; i++){
    ...
    // resize
   dataMtx.resize(n_in, std::vector<bool>(m_in));
    // clear
     for(int i=0; i<m_in; i++){
        for(int j=0; j<n_in; j++){
           dataMtx[i][j]=0.0;
 ...
 * OUTPUTS: 0 on success -1 on fail
 */
int boolSet::columnSet(int n_in, std::vector<double> perturbedColumn){
    n_in=n_in+1;
    if(perturbedColumn.size()!=(unsigned int)m|| n_in<=0 || n_in>n)
        return -1;
    ...
    // loop through all the values
    for(int i=0; i<m; i++){
        this->set_boolMtx(i+1, n_in, perturbedColumn[i]);
    }
